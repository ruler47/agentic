...
const toolReworkWaitStore = pool
  ? new PostgresToolReworkWaitStore(pool)
  : new InMemoryToolReworkWaitStore();
...
const toolBuildWorkflow = new ToolBuildWorkflow(
  ...
  new GeneratedToolFileBuilder(
    [
      ...
      // Messaging service provider runs BEFORE the generic service provider so requests
      // that name a concrete messaging provider get the matching provider adapter
      // instead of a provider-neutral bridge.
