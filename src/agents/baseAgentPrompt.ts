...
export function buildBaseAgentSystemPrompt(
  ...
  return [
    ...
    "If the task frame source policy forbids external research, do not call web.search, web.read, web.extract, http.request, browser.operate, or browser.screenshot. Answer from local/thread context and clearly state what would require fresh external data.",
    "If the task frame includes a source search plan, follow it before improvising search queries. When mixed-language search is required, include both the user's language and English search angles unless the task is explicitly local-language-only.",
