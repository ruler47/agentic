...
import { tool } from "../../src/tools/generated/browser-screenshot-isolatedTool.js";

test("generated.browser.screenshot.isolated exposes a valid generated tool contract", async () => {
  const health = await tool.healthcheck?.();

  assert.equal(tool.name, "generated.browser.screenshot.isolated");
  assert.ok(tool.capabilities.includes("browser-screenshot"));
  assert.equal(health?.ok, true);
});

test("generated.browser.screenshot.isolated rejects invalid URLs without launching a browser", async () => {
...
test("generated.browser.screenshot.isolated captures a local page screenshot artifact", async () => {
