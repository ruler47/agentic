...
import { tool } from "../../src/tools/generated/browser-screenshot-manualTool.js";

test("generated.browser.screenshot.manual exposes a valid generated tool contract", async () => {
  const health = await tool.healthcheck?.();

  assert.equal(tool.name, "generated.browser.screenshot.manual");
  assert.ok(tool.capabilities.includes("browser-screenshot"));
  assert.equal(health?.ok, true);
});

test("generated.browser.screenshot.manual rejects invalid URLs without launching a browser", async () => {
...
test("generated.browser.screenshot.manual captures a local page screenshot artifact", async () => {
