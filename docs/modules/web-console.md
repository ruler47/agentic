...
Worker and reviewer cards also include `payload.invocation` in addition to the historical
`payload.callFrame`. The UI can use that invocation payload to display parent-child
agent calls consistently while the runtime keeps the proven worker/reviewer tool and
artifact code paths intact.
