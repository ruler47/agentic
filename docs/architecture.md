...
lifecycle state in `tool_service_statuses` when Postgres is configured, and exposes the
lifecycle through `/api/tool-services` plus the Channels and Tool Detail pages. It does
not yet spawn durable background processes or own webhook routing; those must be added as
generic runners behind the same tool contract.
