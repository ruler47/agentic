...
The existing worker, reviewer, and synthesizer spans now also carry `payload.invocation`
alongside their older span-specific payloads. This is a compatibility bridge: the current
worker runtime still owns mature tool, artifact, ledger, and review behavior, but every
worker/reviewer/synthesis trace node exposes the same caller, local-task,
