...
Status: **Planned. Operator goal: tool = independent app with its
own image + version, visible in `docker ps` alongside redis,
postgres, etc.**

Today every council-built tool runs as a Node subprocess inside
the main `agentic-app` container — spawned via `spawn(node, …)`
from the bundle runner. That works but it has issues:

- A tool's runtime can crash the parent (we hit this with the
  uncaught `spawn ENOENT` race).
- Tools share the agentic-app container's image, kernel, fs,
  and resource limits.
- `docker ps` shows one `agentic-app-1` container — operators
  cannot see "screenshot.url v1.0.8 is running, here's its CPU".
- Tool isolation is process-level only — no cgroups, no seccomp.
- Cannot move a tool to a different host / cluster without
  moving the whole agentic-app.

Phase 26 turns each tool version into a **first-class Docker
container** that the main app talks to over HTTP/JSON. The
container name encodes name + version so `docker ps` is a live
inventory.

### Slice A — Per-tool Dockerfile + image build

Council adapter writes a `Dockerfile` into the bundle alongside
the existing scaffold:

```dockerfile
# auto-generated by council scaffold
FROM node:22-slim
WORKDIR /app
...
The `registerToolFromFiles` step calls `docker build -t
agentic-tool-<name>-<version>:latest .` after writing files.
Build failures surface in the existing install-verify span
(Phase 25 Slice C) with the docker logs in detail.

### Slice B — Per-tool container start / stop

`SourceBundleDockerToolPackageRunner` (new package runner)
replaces the subprocess runner for council-built tools:

```ts
docker run -d
  --name agentic-tool-<sanitized-name>-<version>
  --label agentic.tool.name=<name>
  --label agentic.tool.version=<version>
  --network agentic_default
  --memory=512m
  --cpus=1.0
  -p 0:8080   # let docker pick a host port
  agentic-tool-<name>-<version>:latest
```

The main app reads the assigned host port via `docker port
<name>` and talks to the tool via the existing `/run` + `/health`
contract.

### Slice C — Container lifecycle

- Idle tools stop after N minutes of no calls (mirror current
  on-demand startupMode).
- `tool-services` page lists docker containers with status +
  CPU + RAM + uptime.
- "Stop tool" / "Restart tool" buttons hit `docker stop` /
  `docker restart` directly.
- Crash → main app emits `tool-service-crashed` event + tries
  one restart with exponential backoff.
- Promotion of a new version: build new image, start new
  container with new name, point routing at it, stop old after
  a grace period.

### Slice D — Migration path

The current source-bundle subprocess runner stays as a fallback
for tools that haven't been re-promoted under the new flow.
Phase 26 ships as a per-instance flag: set
`TOOL_RUNTIME=docker-per-tool` to opt into the new path, leave
the default at `subprocess` until the model is exercised in dev.

### Risks

- Build-per-rework cost. A puppeteer tool builds ~500 MB image.
  At 5 reworks/day that's 2.5 GB/day churn. Mitigation: layer
  the Dockerfile so package.json + npm install is cached, only
  the source layer changes per rework.
- Docker socket access. Main app needs `/var/run/docker.sock`
  mounted to run `docker build` / `docker run`. That's a
  security expansion — operators on shared hosts should review.
- Network. Per-tool containers need to reach the main app for
  callback APIs (artifacts, runtime settings). Use the same
  `agentic_default` network.
- Logs. Tool stdout/stderr live in the container, not the main
  app's logs. The `tool-services/<name>/logs` page needs to
  call `docker logs` instead of reading the runner's in-memory
  buffer.
